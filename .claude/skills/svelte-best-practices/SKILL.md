...
description: >-
  Idiomatic Svelte 5 / SvelteKit patterns: when to use $state vs $derived vs
  $effect, $props reactivity, $inspect.trace debugging, event attributes,
  snippets, keyed each blocks, styling child components, context over shared
  modules, async Svelte, and avoiding legacy (pre-runes) features.
  USE FOR: writing or reviewing Svelte 5 components, deciding between runes,
  effect overuse / state-in-effect smells, event handler syntax, snippet vs
  slot questions, each-block keying, cross-component CSS, migrating legacy
  Svelte code to runes mode.
metadata:
  author: user
  version: "0.0.1"
---
