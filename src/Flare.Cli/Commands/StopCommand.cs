...
internal sealed class StopCommand : AsyncCommand
{
    protected override async Task<int> ExecuteAsync(CommandContext context, CancellationToken cancellationToken)
    {
        if (!FlareHome.IsInitialized)
        {
            AnsiConsole.MarkupLine("[grey]Not initialized yet - nothing to stop. Run `flare start` first.[/]");
            return 0;
        }
        ...
        var exitCode = await ComposeRunner.RunStreamedAsync(["stop"]);
        if (exitCode != 0)
        {
        ...
        AnsiConsole.MarkupLine("[green]✓[/] Stopped. Data volumes were kept - `flare start` picks up where you left off.");
        return 0;
    }
}
