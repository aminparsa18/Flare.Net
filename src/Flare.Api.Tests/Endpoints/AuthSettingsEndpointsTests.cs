...
public class AuthSettingsEndpointsTests
{
    ...
    public async Task Put_EnablesAuth_WhenLocalIsEnabled()
    {
        ...
        var result = await AuthSettingsEndpoints.HandlePutAsync(context, authSettings, new FakeEntraSettingsStore(), new FakeLdapSettingsStore(), new FakeOidcSettingsStore(), new FakeProxyAuthSettingsStore(), CancellationToken.None);
        await result.ExecuteAsync(context);
    ...
    public async Task Put_AllowsEnabling_WithOnlyEntraAsTheUsableMethod()
    {
        ...
        var result = await AuthSettingsEndpoints.HandlePutAsync(context, authSettings, entraSettings, new FakeLdapSettingsStore(), new FakeOidcSettingsStore(), new FakeProxyAuthSettingsStore(), CancellationToken.None);
        await result.ExecuteAsync(context);
    ...
    public async Task Put_AllowsEnabling_WithOnlyActiveDirectoryAsTheUsableMethod()
    {
        ...
        var result = await AuthSettingsEndpoints.HandlePutAsync(context, authSettings, new FakeEntraSettingsStore(), ldapSettings, new FakeOidcSettingsStore(), new FakeProxyAuthSettingsStore(), CancellationToken.None);
        await result.ExecuteAsync(context);
    ...
    public async Task Put_AllowsEnabling_WithOnlyOidcAsTheUsableMethod()
    {
        ...
        var result = await AuthSettingsEndpoints.HandlePutAsync(context, authSettings, new FakeEntraSettingsStore(), new FakeLdapSettingsStore(), oidcSettings, new FakeProxyAuthSettingsStore(), CancellationToken.None);
        await result.ExecuteAsync(context);
    ...
    [Fact]
    public async Task Put_AllowsEnabling_WithOnlyProxyAuthAsTheUsableMethod()
    {
        var authSettings = new FakeAuthSettingsStore(enabled: false, localEnabled: false);
        var proxyAuthSettings = new FakeProxyAuthSettingsStore(new Flare.Identity.Auth.ProxyAuthSettings(
            true, "Remote-User", "172.18.0.0/16", null, null, null, null,
            Flare.Identity.Users.UserRole.Viewer, DateTimeOffset.UtcNow));
        var context = CreateContext(new { enabled = true, localEnabled = false });

        var result = await AuthSettingsEndpoints.HandlePutAsync(context, authSettings, new FakeEntraSettingsStore(), new FakeLdapSettingsStore(), new FakeOidcSettingsStore(), proxyAuthSettings, CancellationToken.None);
        await result.ExecuteAsync(context);
