...
/// discovery: every distinct metric name in scope, across all four point-type tables,
...
/// "one table per point type" design decision). The same <see cref="MetricFilterSqlBuilder"/>
/// output is reused verbatim in every branch (and its parameters bound once, not
/// duplicated per branch) - it's schema-identical across the tables' shared
/// <c>Time</c>/<c>ServiceName</c>/<c>DataPointAttributes</c> columns.
///
/// The branches are wrapped in a subquery before the final <c>ORDER BY</c> is
...
public static class MetricNamesQueryBuilder
{
    public static MetricNamesSql Build(MetricNamesRequest request, DateTimeOffset now)
    {
        ...
        var sql = "SELECT * FROM (\n" +
            ...
            "UNION ALL\n" +
            $"SELECT MetricName, ServiceName, 'exponential_histogram' AS Type, any(Unit) AS Unit, any(Description) AS Description, count(DISTINCT toString(DataPointAttributes)) AS SeriesCount FROM metrics_exponential_histogram WHERE {filterSql.WhereSql} GROUP BY MetricName, ServiceName\n" +
            ")\n" +
