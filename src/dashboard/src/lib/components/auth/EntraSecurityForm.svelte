<script lang="ts">
	...
	// paste in their own Entra ID App Registration values instead of editing
	// docker-compose/.env, mirroring Seq's own Security settings page (the reference
	// this feature was scoped from - see docs/auth.md's "Microsoft Entra ID (SSO)"
	// section). Settings only take effect after a Flare.Api restart - deliberately not
	// attempting to detect/poll for that, just saying so. Formerly its own page
	// (SecuritySettingsForm.svelte at /security) - moved here, unchanged otherwise, when
	// /security and /users were consolidated into /auth.
