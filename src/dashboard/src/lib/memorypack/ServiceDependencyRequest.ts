...
// `ServiceDependencyRequest` field-for-field - same shape and same reason it can't carry
// `[GenerateTypeScript]` as `ServiceOverviewRequest`.

import { MemoryPackWriter } from '$lib/generated/memorypack/MemoryPackWriter.js';
import { MemoryPackReader } from '$lib/generated/memorypack/MemoryPackReader.js';
import { ResourceAttributeFilter } from '$lib/generated/memorypack/ResourceAttributeFilter.js';

export class ServiceDependencyRequest {
	...
	static serialize(value: ServiceDependencyRequest | null): Uint8Array {
		const writer = MemoryPackWriter.getSharedInstance();
		this.serializeCore(writer, value);
		return writer.toArray();
	}

	static serializeCore(writer: MemoryPackWriter, value: ServiceDependencyRequest | null): void {
		if (value == null) {
	...
	static deserialize(buffer: ArrayBuffer): ServiceDependencyRequest | null {
		return this.deserializeCore(new MemoryPackReader(buffer));
	}

	static deserializeCore(reader: MemoryPackReader): ServiceDependencyRequest | null {
		const [ok, count] = reader.tryReadObjectHeader();
		if (!ok) {
			return null;
		}

		const value = new ServiceDependencyRequest();
		if (count == 2) {
