<!-- Translated (zh-CN) from docs/tutorials/getting-started.md · source-sha256:d04e0a8ddcef43e9 · hand-translated by Claude, not generated by scripts/translate-docs.py - edit the source file and retranslate if it changes, don't machine-retranslate over this. -->
...
> 不想为了启动堆栈而克隆整个仓库？`curl -fsSL
> https://raw.githubusercontent.com/aminparsa18/Flare.Net/main/scripts/install.sh
> | bash` 会在缺少 Docker 时自动安装它，然后改为从已发布的镜像启动同一个堆栈 —— 参见
> [`../how-to/run-standalone.md#零前提快速安装`](../how-to/run-standalone.zh-CN.md#零前提快速安装)。
> 不管怎样，步骤 2 仍然需要 .NET SDK，因为本教程的这个一次性控制台应用正是由它搭建的。
