...
через один или несколько `/notification-channels` — многоразовые
именованные адресаты (webhook/Slack, Telegram, email или PagerDuty), на
которые правило ссылается по ID вместо встраивания напрямую, так что
один и тот же канал можно переиспользовать между правилами, а одно
правило может уведомлять сразу несколько адресатов при одном и том же
нарушении (см.
