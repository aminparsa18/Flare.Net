...
задержка p99 и объём ожидающих сообщений: отставание потребителей Kafka или
глубина очередей RabbitMQ. Скорости и задержки вычисляются во время запроса
из атрибутов OTel `messaging.*` спанов производителей и потребителей,
поэтому отдельный агент или таблица не нужны; объём ожидающих сообщений
берётся из ресивера `kafkametrics` или `rabbitmq` коллектора, если он
настроен. Нажмите на строку, чтобы увидеть производителей, потребителей,
партиции, отставание по группам или глубину по очередям. См. [`../how-to/monitor-message-queues.ru.md`](../how-to/monitor-message-queues.ru.md),
...
«проверить на текущих данных» перед сохранением. По умолчанию каждое правило
проверяется на каждом цикле опроса воркера оповещений (30 с), а для медленных
и дорогих правил можно задать собственный интервал «Evaluate every»
(от 1 мин до 1 ч). Правило по метрике может также требовать минимальное
число точек данных в окне; если их меньше, оно сообщает о недостатке данных
и не срабатывает. Правило также может заменить заголовок и текст своего
уведомления шаблоном с подстановками вроде `{{value}}`, `{{threshold}}`,
`{{logs_url}}` или `{{labels.service.name}}`, с живым предпросмотром в форме
правила. Если настроен публичный URL, каждое уведомление содержит ссылку на
данные, вызвавшие срабатывание: подходящие логи, график метрики или подходящие
исключения, ограниченные фильтром правила за оценённое окно (`{{data_url}}` в
шаблоне). Вкладка «Maintenance» планирует разовые, ежедневные или
еженедельные окна обслуживания (в выбранном часовом поясе) для всех правил
или выбранных. Во время окна правила по-прежнему вычисляются, но
срабатывание записывается в историю правила как подавленное вместо
уведомления (см. [ADR-0055](../../docs-internal/adr/0055-alert-maintenance-windows.md)). Срабатывание уведомляет
через один или несколько каналов уведомлений, управляемых со вкладки
«Channels» этой же страницы (или создаваемых прямо из формы правила
кнопкой «New channel») — многоразовые именованные адресаты
(webhook/Slack, Telegram, email или PagerDuty), на которые правило
ссылается по ID вместо встраивания напрямую, так что один и тот же
канал можно переиспользовать между правилами, а одно правило может
уведомлять сразу несколько адресатов при одном и том же нарушении (см.
[ADR-0021](../../docs-internal/adr/0021-reusable-notification-channels.md)).
Правило, созданное до появления этой возможности, по-прежнему
уведомляет через собственный встроенный канал без изменений. См.
[`../../src/Flare.Api/README.md`](../../src/Flare.Api/README.md#alerting)
о том, что нужно настроить на стороне сервера для каждого типа канала.
