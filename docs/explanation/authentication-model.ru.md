...
Аналог ключей API приёма данных, но для стороны *запросов*, а не
приёма: персональный токен доступа (PAT) — это долговечный
токен-предъявитель, который скрипт, CI-задача или другой сервис могут
предъявить вместо сеанса браузера, чтобы вызывать `/api/logs`,
`/api/alerts` и т. д., не выдавая себя за чужую cookie. В отличие от
ключей приёма, PAT намеренно **привязан к конкретному пользователю,
создаётся самостоятельно и не даёт больше прав, чем есть у создавшего
его пользователя** — создание токена не является действием
администратора, и токен аутентифицируется как тот самый пользователь
(с той же ролью, теми же правами), а не как отдельная машинная
identity. См.
[ADR-0019](../../docs-internal/adr/0019-personal-access-tokens.md) о
том, почему это переиспользует существующую схему аутентификации
сеансов, а не регистрирует вторую, и
[практическое руководство](../how-to/configure-authentication.ru.md#персональные-токены-доступа)
о создании, использовании и отзыве токена.

Каждый токен также индивидуально ограничен по частоте запросов —
`Auth:PatRateLimitPermitLimit` запросов (по умолчанию 120) за
`Auth:PatRateLimitWindow` (по умолчанию 1 минута), независимо от
ограничений на выполнение отдельного запроса (`max_execution_time`,
`max_rows_to_read` и т. д.), которые уже ограничивают стоимость одного
запроса. Токен, превысивший лимит, получает `429` с заголовком
`Retry-After` до сброса окна; обычный сеанс дашборда (аутентификация по
cookie) этим никогда не затрагивается, сколько бы запросов он ни делал.
Лимит хранится в памяти процесса, и это не пробел, а осознанный выбор:
`Flare.Api` и так работает только в одной реплике (см.
[ADR-0004](../../docs-internal/adr/0004-embedded-sqlite-for-identity.md)),
поэтому нет второго процесса, с которым это состояние нужно было бы
согласовывать. Полное решение см. в
...
Единственный метод без внешнего провайдера идентичности за ним — проверка
пароля происходит целиком внутри `Flare.Identity`, по bcrypt-хешу,
хранящемуся рядом со строкой `Users` учётной записи. Та же позиция против
перечисления пользователей, что и у LDAP: `POST /api/auth/login`
объединяет «такого имени пользователя нет», «неверный пароль» и
«учётная запись отключена» в один и тот же общий `401`, никогда не
раскрывая, что произошло на самом деле.

**Неудачные попытки ограничиваются по паре (имя пользователя, IP
клиента)**, а не только по имени пользователя — злоумышленник, долбящий
одну учётную запись с одного источника, получает блокировку (`429`, с
заголовком `Retry-After`), но кто-то другой, законно ошибающийся в своём
собственном пароле несколько раз, не блокируется попутно из-за не
связанной с ним атаки на то же имя пользователя с другого источника.
Настраивается через `Auth:MaxFailedLoginAttempts` (по умолчанию 5),
`Auth:LoginLockoutDuration` (по умолчанию 15 минут) и
`Auth:LoginFailureWindow` (по умолчанию 15 минут — как долго неудачная
попытка остаётся достаточно «недавней», чтобы учитываться в пороге); см.
[справочник конфигурации](../reference/authentication-config.ru.md#справочник-конфигурации).
Успешный вход очищает отслеживаемые неудачи для этой пары. «IP клиента» —
это необработанный TCP-пир запроса, тот же осознанный выбор, что и у
`ProxyAuthLoginEndpoints`/`TrustedProxyNetworks` для аутентификации через
обратный прокси: доверие переданному заголовку здесь позволило бы
злоумышленнику подделывать новый IP на каждой попытке и полностью обойти
блокировку. **Известное ограничение:** за обратным прокси каждый запрос
поэтому делит IP самого прокси для целей ограничения — принято, а не
упущено, по той же причине, по которой заголовку не доверяют и для
идентичности.
