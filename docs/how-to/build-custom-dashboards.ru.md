<!-- Translated (ru) from docs/how-to/build-custom-dashboards.md · source-sha256:ce7c81da90957407 · hand-translated by Claude, not generated by scripts/translate-docs.py - edit the source file and retranslate if it changes, don't machine-retranslate over this. -->
...
Пока нет цепочек переменных (сужение опций одной переменной в зависимости
от выбора другой) и нет отключения переменной для отдельной панели — см.
...
- **Нет цепочек переменных** — значения переменной «Из запроса» всегда
  определяются независимо, за то же фиксированное окно в 7 дней; одна
  переменная не может сузить свои варианты в зависимости от текущего
  выбора другой переменной.
- **Нет отключения переменной для отдельной панели** — переменная сужает
  каждую панель, к которой применима её цель/тип атрибута, для всего
  дашборда; исключить из этого одну панель нельзя.
