...
## Переопределение сервиса на время сессии

Селектор **сервиса** в заголовке дашборда (рядом с переопределением
временного диапазона) временно сужает все панели Logs/Metrics/Traces до
одного сервиса, независимо от того, с каким сервисом каждая панель была
закреплена/добавлена. Верните значение **Сервис каждой панели**, чтобы
снова показывать сервис(ы), с которыми была сохранена каждая панель. Как
и переопределение временного диапазона, это действует только в рамках
браузера/сессии — никогда не сохраняется в дашборде. Это фиксированная,
встроенная переменная, а не сохранённая или основанная на запросе: она
всегда означает «сервис», и пока нет возможности определить собственную
переменную на основе произвольного запроса.
...
- **Импортировать** дашборд — выберите JSON-файл, ранее созданный функцией
  «Экспортировать» (на этом экземпляре или на другом), чтобы создать из
  него новый дашборд. Понимается только собственный формат экспорта Flare,
  а не JSON дашборда Grafana (см. «Известные ограничения» ниже); неверный
  или посторонний файл отклоняется с ошибкой на экране, а не импортируется
  частично.
...
- **Нет импорта JSON-дашборда Grafana** — «Импортировать» понимает только
  собственный формат экспорта Flare (см. «Управление дашбордами» выше).
