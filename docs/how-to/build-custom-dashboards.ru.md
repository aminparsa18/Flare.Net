...
| **Столбчатая с накоплением** | Ряды каждого интервала сложены в один столбец, высота которого равна сумме интервала. |
...
- Столбцы и числа метрики Sum — это исходные счётчики за интервал, а не
  ...
  умолчанию.
- Метрика Histogram использует **среднее** каждого интервала (сумма ÷
...
число, а в таблице — подходящие ячейки.
...
- **Импортировать** дашборд — выберите JSON-файл, чтобы создать из него
  ...
  - Экспорт дашборда Grafana («Export as JSON» из интерфейса Grafana, или
    ...
    **Столбчатой диаграммой**, pie chart — **Круговой диаграммой**. **Запросы не переносятся** — запрос панели Grafana написан для
...
- **Импорт из Grafana — только структурный** — компоновка и тип панели
  ...
  «Группировка панелей в строки»).
- **Визуализации есть только у панелей Metrics, без настроек по столбцам** —
  панели Logs и Traces не могут менять визуализацию. Все столбцы таблицы
  используют единицу метрики (без отдельной единицы для столбца), а
  визуализации в виде гистограммы распределения значений пока нет.
