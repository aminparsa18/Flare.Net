...
| Consumer lag | Только Kafka, см. [ниже](#отставание-потребителей-kafka) |
...
Столбец **Consumer lag** показывает последнее значение отставания каждого
топика за период, суммированное по всем группам потребителей и партициям.
**—** означает, что метрика для этого топика не собирается. Как 0 она никогда
не показывается.
...
`messaging.destination.name`, например при публикации в exchange RabbitMQ по
умолчанию.
