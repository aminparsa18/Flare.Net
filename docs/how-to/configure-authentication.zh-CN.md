<!-- Auto-translated (zh-CN) from docs/how-to/configure-authentication.md · source-sha256:688bf7340446d492 · generated by scripts/translate-docs.py - edit the source file and rerun the script, don't hand-edit this one. -->
...
个人访问令牌以特定*用户*的身份对查询 API（`/api/logs`、`/api/alerts` 等）进行身份验证，与上面的摄取 API 密钥是分开的。自助式 — 任何已登录的用户（`Viewer` 及以上）都可以创建自己的令牌；无需管理员操作。

- **创建一个**：登录后调用 `POST /api/access-tokens`，例如通过仪表板的终端（打开它，运行 `help` 查看确切命令）— `{"name": "ci-pipeline", "expiresInDays": 90}`（`expiresInDays` 是可选的；省略则令牌永不过期）。原始令牌**仅显示一次** - 立即将其复制到安全的地方。
- **使用它**：在任何查询 API 请求上发送 `Authorization: Bearer <token>`。它以创建者的身份进行认证，拥有该用户现有的角色 - 它不会授予该用户原本没有的任何权限。
- **列出/撤销**：`GET /api/access-tokens` 列出你自己的令牌；`DELETE /api/access-tokens/{id}` 立即撤销一个（`Admin` 也可以按 id 撤销属于其他用户的令牌，而无需禁用该用户的整个帐户）。
- **不适用于实时追踪 WebSocket** - 浏览器无法像自动发送会话 Cookie 那样为 WebSocket 升级附加自定义标头。对于实时追踪，请使用会话（即保持登录状态）；PAT 用于普通的请求/响应调用。
