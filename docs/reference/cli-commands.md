...
| `flare alerts list` / `flare alerts test <ID>` / `flare alerts send-test <ID>` | `list` tables saved alert rules (name, enabled, threshold, window, notification channel — webhook/Slack, Telegram, Email, or PagerDuty) via `Flare.Api`'s `GET /api/alerts`. `test <ID>` dry-runs a saved rule's condition/threshold against current data via `POST /api/alerts/{id}/test` — **ignores cooldown, sends no notification** — safe to run repeatedly without triggering anything real. If the rule has absent-data alerting enabled and its condition matched nothing over that window, it prints **No data** (and the no-data window) instead of an observed count. For an anomaly rule it prints the observed value, the baseline mean and the z-score, or **Not enough history** when fewer than 3 baseline windows had data. `list` shows an anomaly rule's threshold as its z-score and baseline (e.g. `|z| >= 3 vs 7 days`). `send-test <ID>` actually sends a test notification through the rule's configured channel via `POST /api/alerts/{id}/send-test`, to verify a Slack/webhook/email/Telegram/PagerDuty channel is wired correctly before relying on it for a real breach. |
...
| `flare notification-channels list` / `create <NAME> --type T ...` / `update <ID> [...]` / `delete <ID> [--yes]` / `send-test <ID>` | CRUD for saved [`NotificationChannel`](../../docs-internal/adr/0021-reusable-notification-channels.md)s via `Flare.Api`'s `/api/notification-channels` — the CLI-native equivalent of the dashboard's Alerts → Channels tab, for wiring channels without clicking through the UI. `create`/`update` take `--type webhook\|telegram\|email\|pagerduty` plus the matching destination flag(s) (`--webhook-url`, `--telegram-bot-token`+`--telegram-chat-id`, `--email-to`, or `--pagerduty-routing-key`) and `--description`; `update <ID>` fetches the existing channel first and only overrides flags you actually pass, so e.g. `update <ID> --rename new-name` alone doesn't require re-entering the destination (`--rename`, not `--name` — that's already the instance-targeting flag every command shares). `delete` prompts for confirmation unless `--yes`. `send-test` fires a synthetic (non-breaching) test notification through that one channel, independent of any alert rule. `flare alerts list`'s own rule summary still shows only a referenced channel count, not names — resolving `ChannelIds` to names there remains a follow-up. |
| `flare traces [-s service]... [--status s]... [--kind k]... [--trace-id id] [--attr key=value]... [--attr-not key=value]... [--attr-exists key]... [--attr-absent key]... [--min-duration d] [--max-duration d] [--since range] [--limit count]` | Searches recent traces (root spans) via `Flare.Api`'s Query API — the CLI-native equivalent of the dashboard's Trace List. `--status` accepts `ok`/`error`/`unset`, repeatable. `--kind` accepts `internal`/`server`/`client`/`producer`/`consumer`, repeatable. `--attr`/`--attr-not`/`--attr-exists`/`--attr-absent` filter the span attribute bag (Equals/NotEquals/Exists/Absent); repeatable and ANDed together — always against the default `Span` bag, no way to target `Resource`/`Scope` from the CLI yet. `--min-duration`/`--max-duration` take e.g. `500ms`/`2s`/`1.5m`. `--since` defaults to `1h` (e.g. `15m`, `6h`, `24h`, `7d`). One-shot only, no live-tail equivalent. |
